# scripts/gm_extract_features_cpu_fixed_v11.py
import torch
    ...
    def process_video(self, path, fps_target=1):
        ...
        while True:
            ...
            if int(cap.get(cv2.CAP_PROP_POS_FRAMES)) % interval == 0:
                ...
                # 3. FIX DU BUG NUMPY/TORCH
                # On convertit en array, puis on force une copie contiguë
                img_np = np.array(img_resized, dtype=np.float32) / 255.0
                
                # C'est ICI que ça change : on utilise torch.tensor() au lieu de from_numpy()
                # Cela force une copie des données et ignore les conflits de version d'ABI
                tensor = torch.tensor(img_np)
                
                # 4. Suite normale (HWC -> CHW et Normalisation)
    ...
    with h5py.File(hdf5_path, "a") as hf:
        for v_path in tqdm(video_paths, desc="Extraction"):
            ...
            if features is not None:
                ...
                logger.info(f"✅ {v_path.name} : {features.shape[0]} frames")
            else:
                logger.warning(f"⚠️ {v_path.name} : Vide")
