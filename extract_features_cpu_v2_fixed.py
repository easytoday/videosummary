    ...
    def extract_features_from_video_optimized(self, video_path, target_fps=1):
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                ...
                try:
                    # Récupérer le dtype de manière safe
                    frame_dtype = getattr(frame, 'dtype', None)
                    
                    # Si ce n'est pas uint8 et que c'est un ndarray, convertir
                    if frame_dtype != np.uint8 and isinstance(frame, np.ndarray):
                        try:
                            logger.debug(f"Frame {frame_idx} conversion dtype: {frame_dtype} -> uint8")
                            
                            # Gérer différents cas de conversion
                            if frame_dtype in [np.float32, np.float64]:
                                # Normaliser les floats (0-1 ou 0-255)
                                if frame.max() <= 1.0:
                                    frame = (frame * 255).clip(0, 255).astype(np.uint8)
                                else:
                                    frame = frame.astype(np.uint8)
                            elif frame_dtype == np.int32 or frame_dtype == np.int64:
                                # Gérer les entiers
                ...
                # Vérifier que le dtype est maintenant uint8
                if hasattr(frame, 'dtype') and frame.dtype != np.uint8:
                    logger.warning(f"Frame {frame_idx} dtype final invalide: {frame.dtype}")
                    consecutive_errors += 1
                    if consecutive_errors > max_consecutive_errors:
                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt")
                        break
