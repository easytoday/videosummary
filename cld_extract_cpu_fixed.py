    ...
    def extract_features_from_video_optimized(self, video_path, target_fps=1):
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                # CORRECTION PRINCIPALE: Vérifier que frame est valide
                if frame is None:
                ...
                # CORRECTION: Vérifier que frame est bien un numpy array
                if not isinstance(frame, np.ndarray):
                    logger.warning(f"Frame {frame_idx} n'est pas un np.ndarray (type={type(frame)}), ignorée")
                    consecutive_errors += 1
                    if consecutive_errors > max_consecutive_errors:
                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt")
                        break
                ...
                # CORRECTION: Forcer la conversion en uint8 si nécessaire
                if frame.dtype != np.uint8:
                    try:
                        # Normaliser d'abord si les valeurs sont hors [0, 255]
                        if frame.max() > 255 or frame.min() < 0:
                            frame = np.clip(frame, 0, 255)
                        frame = frame.astype(np.uint8)
                    except Exception as e:
                        logger.warning(f"Frame {frame_idx} conversion uint8 échouée: {e}")
                        consecutive_errors += 1
                        if consecutive_errors > max_consecutive_errors:
                            logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt")
                            break
