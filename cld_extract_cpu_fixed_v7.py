    ...
    def extract_features_from_video_optimized(self, video_path, target_fps=1):
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                ...
                # Conversion manuelle complète pour éviter PIL (qui a un bug de compatibilité)
                try:
                    # Redimensionner manuellement avec cv2 (pas de PIL)
                    frame_resized = cv2.resize(frame_rgb, (224, 224), interpolation=cv2.INTER_LINEAR)
                    
                    # S'assurer que c'est float32 et normaliser [0, 255] -> [0, 1]
                    frame_float = frame_resized.astype(np.float32) / 255.0
                    
                    # Normalisation ImageNet manuelle
                    ...
                    logger.debug(f"Frame {frame_idx} - Transformation manuelle réussie! Shape: {frame_tensor.shape}")
                    
                except Exception as e:
                    logger.warning(f"Erreur transformation manuelle frame {frame_idx}: {e}")
