    ...
    def extract_features_from_video_optimized(self, video_path, target_fps=1):
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                ...
                # CORRECTION CRITIQUE: Forcer une copie contiguë en mémoire
                # OpenCV semble retourner quelque chose d'inhabituel
                try:
                    # Créer une vraie copie numpy contiguë
                    frame = np.array(frame, dtype=np.uint8, copy=True, order='C')
                    
                    # Vérifier que c'est bien devenu un numpy array
                    if not isinstance(frame, np.ndarray):
                        logger.warning(f"Frame {frame_idx} échec création numpy array, ignorée")
                        consecutive_errors += 1
                        if consecutive_errors > max_consecutive_errors:
                            logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt")
                            break
                    ...
                    logger.warning(f"Frame {frame_idx} conversion numpy échouée: {e}")
                    consecutive_errors += 1
                    if consecutive_errors > max_consecutive_errors:
                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt")
                        break
                ...
                # Conversion BGR -> RGB (frame est maintenant un vrai numpy array uint8 contiguë)
                try:
