...
def main():
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx in idxs:
            ...
            #seq = torch.from_numpy(seq).unsqueeze(0) # input shape (1, seq_len, dim) #TypeError: expected np.ndarray (got numpy.ndarray)
            # correction de l'erreur NumPy
            #seq = torch.from_numpy(np.array(seq, dtype=np.float32)).unsqueeze(0)
            # correction par passage à version 1.26 de NumPy - maj de env projet_h5fix
...
def evaluate(model, dataset, test_keys, use_gpu):
    ...
    with torch.no_grad():
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            #seq = torch.from_numpy(np.array(seq, dtype=np.float32)).unsqueeze(0) # Aziz pour éviter pb NumPy
            seq = torch.FloatTensor(seq).unsqueeze(0) # Aziz nouvelle correction
            if use_gpu: seq = seq.cuda()
            probs = model(seq)
