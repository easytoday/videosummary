...
import time
import logging
import traceback # remonte jusqu'à l'erreur
...
class CPUOptimizedGoogLeNetExtractor:
    ...
    def __init__(self, batch_size=8):
        ...
        # (attention aux warnings sur l'API 'pretrained')
        #self.model = models.googlenet(pretrained=True) # Aziz - cette ligne pose problème
    ...
    def _create_feature_extractor(self, model):
        ...
        # GoogLeNet a une architecture composée : on récupère le backbone et on force
        # un pooling en sortie pour obtenir (B, 1024)
    ...
    def extract_features_from_video_optimized(self, video_path, target_fps=1):
        ...
        # Sauter les videos problématiques après trop d'erreurs - Aziz
        consecutive_errors = 0
        max_consecutive_errors = 10 # Arrêter après 10 erreurs consécutives

        # Barre de progression (total peut être 0)
        ...
        while True:
            ...
            if frame_idx % frame_interval == 0:
                ...
                if frame is None:
                    ...
                    consecutive_errors += 1 # Aziz
                    if consecutive_errors > max_consecutive_errors:  # Aziz 
                ...
                # Vérification explicite pour les frame 1x1 -  Aziz - correction debug taille frame
                ...
                # Filtrer les très petites images (PIL problématique sur 1x1 etc.)
                # if h < 2 or w < 2:
                if h <= 1 or w <= 1:  # Aziz - plus explicite
                ...
                # Vérifier le dtype avant la conversion de BGR à RGB
#                if frame.dtype != np.uint8:
#                    #logger.warning(f"Frame {frame_idx} ignorée (dtype={frame.dtype}, attendu uint8)")
#                    logger.warning(f"Frame {frame_idx} ignorée (type={type(frame)}, hasattr_dtype={hasattr(frame, 'dtype')})")
...
#                    continue
#
#                # Vérifier le dtype si est Objet
#                if frame.dtype == np.object_:
#                    logger.warning(f"Frame {frame_idx} ignorée (dtype object)")
#                    consecutive_errors += 1
#                    if consecutive_errors > max_consecutive_errors:
#                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt du traitement de cette vidéo")
#                        break
...
#                    continue
                ...
                # 1. Vérification de base de l'intégrité
                if not isinstance(frame, np.ndarray):
                    logger.warning(f"Frame {frame_idx} n'est pas un array numpy valide")
                    consecutive_errors += 1
                    if consecutive_errors > max_consecutive_errors: break
                    pbar.update(1); frame_idx += 1; continue
                
                # 2. Gestion intelligente du type de données (dtype)
                # Si ce n'est pas du uint8 (8-bit), on force la conversion au lieu de rejeter
                if frame.dtype != np.uint8:
                    try:
                        # On convertit en uint8 en s'assurant de clipper les valeurs si nécessaire
                        frame = np.clip(frame, 0, 255).astype(np.uint8)
                        logger.debug(f"Frame {frame_idx} convertie avec succès de {frame.dtype} vers uint8")
                    except Exception as e:
                        logger.warning(f"Frame {frame_idx} (type {frame.dtype}) conversion impossible : {e}")
                        consecutive_errors += 1
                        if consecutive_errors > max_consecutive_errors: break
                        pbar.update(1); frame_idx += 1; continue
                
                # 3. Conversion de l'espace couleur (BGR vers RGB)
                try:
                    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    consecutive_errors = 0 # On réinitialise ici car on a réussi à traiter la frame
                except Exception as e:
                ...
                #    except Exception as e:
                #        logger.warning(f"Impossible de caster frame {frame_idx} en uint8: {e}")
                #        pbar.update(1)
                #        frame_idx += 1
                #        continue
                ...
                # Assurer type uint8 avec gestion du dtype object- Aziz
                if frame_rgb.dtype != np.uint8:
                    try:
                        # Forcer la conversion et s'assurer que c'est uint8
                        if frame_rgb.dtype == np.object_:
                            # Cas spécial pour dtype object
                            frame_rgb = frame_rgb.astype(np.uint8, copy=True)
                        else:
                            frame_rgb = frame_rgb.astype(np.uint8)
                    except Exception as e:
                        logger.warning(
                            f"Impossible de caster frame {frame_idx} en uint8: {e}, dtype={frame_rgb.dtype}"
                        )
                        consecutive_errors += 1 # Aziz
                        if consecutive_errors > max_consecutive_errors:  # Aziz 
                            logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt du traitement de cette vidéo")
                ...
                # Validation finale avant PIL
                if frame_rgb.dtype != np.uint8:
                    logger.warning(f"Frame {frame_idx} a un dtype invalide: {frame_rgb.dtype}, conversion forcée")
                    consecutive_errors += 1 # Aziz
                    if consecutive_errors > max_consecutive_errors:  # Aziz 
                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt du traitement de cette vidéo")
                        break  # Aziz 
                ...
                # Vérifier la forme une dernière fois
                ...
                try:
                    # DEBUG: Afficher les infos sur la frame avant conversion
                    logger.debug(f"Frame {frame_idx} avant PIL: shape={frame_rgb.shape}, dtype={frame_rgb.dtype}, min={frame_rgb.min()}, max={frame_rgb.max()}")

                    image = Image.fromarray(frame_rgb)
                    tensor = self.transform(image)
                    frames_batch.append(tensor)
                    consecutive_errors = 0 # Aziz Réinitialisation après succès
                except Exception as e:
                    logger.warning(f"Erreur transformation frame {frame_idx}: {e}")
                    logger.error(f"Détails frame: shape={frame_rgb.shape if 'frame_rgb' in locals() else 'N/A'}, dtype={frame_rgb.dtype if 'frame_rgb' in locals() else 'N/A'}, type={type(frame_rgb) if 'frame_rgb' in locals() else 'N/A'}")
                    consecutive_errors += 1 # Aziz
                    if consecutive_errors > max_consecutive_errors:  # Aziz 
                        logger.error(f"Trop d'erreurs consécutives ({consecutive_errors}), arrêt du traitement de cette vidéo")
                        break  # Aziz 
        ...
        try:
            all_features = np.vstack(features_list)  # (n_frames, feature_dim)
        except Exception as e:
    ...
    def process_videos_to_hdf5(
        ...
        with h5py.File(hdf5_path, mode) as hf:
            ...
            if "metadata" not in hf:
                ...
                # Créer datasets de métadonnées extensibles - Aziz correction bug
                ...
                # metadata.group["video_ids"].resize((0,))
                ...
                # metadata.group["video_names"].resize((0,))
                ...
                # h5py n'autorise pas création dataset vide
                # metadata_group.create_dataset(
                #    "frame_counts",
                #    shape=(0,),
                #    maxshape=(None,),
                ...
                # metadata.group["frame_counts"].resize((0,))
                ...
                # Créer datasets de métadonnées extensibles
                # metadata_group.create_dataset(
                #    "video_ids",
                ...
                # )
                # metadata_group.create_dataset(
                #    "frame_counts", data=np.array([], dtype=np.int32), maxshape=(None,)
                # )

                ## Données statiques
                # metadata_group.create_dataset(
                #    "created_date",
                #    data=np.array([str(timestamp)], dtype=h5py.string_dtype(encoding="utf-8")),
                # )
                # metadata_group.create_dataset(
                #    "extractor",
                #    data=np.array(["GoogLeNet (CPU optimized)"], dtype=h5py.string_dtype(encoding="utf-8")),
                # )
                # metadata_group.create_dataset(
                #    "target_fps", data=np.array([float(target_fps)], dtype=np.float32)
                # )
                # metadata_group.create_dataset("feature_dim", data=np.array([1024], dtype=np.int32))
            else:
            ...
            for i, video_path in enumerate(video_files, 1):
                ...
                try:
                    ...
                    # Stocker les features avec compression; chunks devrait être une tuple
                    ...
                    for key, value in metadata.items():
                        try:
                            ...
                            # Sauter les métadonnées non sérialisables
                            logger.debug(
            ...
            if all_frame_counts:
                ...
                # if "total_size_gb" in metadata_group:
                #    del metadata_group["total_size_gb"]
                # metadata_group.create_dataset("total_size_gb", data=np.array([total_size_mb / 1024], dtype=np.float32))
    ...
    def _update_global_metadata(
        ...
        for dset_name, value in [
            ...
            if dset_name in metadata_group:
                ...
                # Pour les strings, écrire en tant que vlen
                if isinstance(value, str):
                    dset[-1] = np.array(
                ...
                if isinstance(value, str):
                    ...
                    # data = np.array([value], dtype=h5py.string_dtype(encoding="utf-8"))
                    # metadata_group.create_dataset(dset_name, data=data, maxshape=(None,))
