# scripts/gm_extract_features_cpu_fixed_v12.py
import torch
...
class FeatureExtractor:
    def __init__(self, batch_size=16):
        ...
        # --- CORRECTIF MAJEUR ---
        # On désactive aux_logits pour éviter le crash de dimension (RuntimeError)
        # Le chargement des poids va peut-être afficher un warning "unexpected keys", c'est NORMAL.
        model = googlenet(weights=GoogLeNet_Weights.DEFAULT, aux_logits=False)
        
        # On recrée une séquence propre sans la couche de classification finale (fc)
        # Comme aux_logits=False, la liste children() est propre et séquentielle.
    ...
    def process_video(self, path, fps_target=1):
        ...
        while True:
            ...
            if int(cap.get(cv2.CAP_PROP_POS_FRAMES)) % interval == 0:
                ...
                # 3. Conversion Manuel (Anti-Bug Numpy 2.0)
                img_np = np.array(img_resized, dtype=np.float32) / 255.0
                tensor = torch.tensor(img_np) # Copie sûre
                
                # 4. Formattage (HWC -> CHW) et Normalisation
...
if __name__ == "__main__":
    ...
    if args.force and hdf5_path.exists():
        os.remove(hdf5_path)
        logger.info(f"♻️  Fichier supprimé : {hdf5_path}")
    ...
    with h5py.File(hdf5_path, "a") as hf:
        for v_path in tqdm(video_paths, desc="Extraction"):
            ...
            if features is not None:
                ...
                logger.info(f"✅ {v_path.name} : {features.shape[0]} vecteurs extraits")
            else:
